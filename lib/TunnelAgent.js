...
class TunnelAgent extends Agent {
    constructor(options = {}) {
        ...
        this.connsDebug = Debug(`ShockConnectionDebug${options.clientId}`)
    ...
    listen() {
        ...
        server.on('error', (err) => {
            this.connsDebug("an error on a connection occurred")
    ...
    _onClose() {
        this.connsDebug("connections closed tcp")
        this.closed = true;
    ...
    _onConnection(socket) {
        this.connsDebug(`new connection ${this.debugCounter++}`);
        // no more socket connections allowed
        if (this.connectedSockets >= this.maxTcpSockets) {
            this.connsDebug(`moar?`);
            this.debug(`no more sockets allowed ${this.connectedSockets}/${this.maxTcpSockets}`);
            socket.destroy();
            return false;
        }
        ...
        socket.once('close', (hadError) => {
            ...
            this.connsDebug(`killing connection ${this.debugCounter--}`);
        ...
        if (fn) {
            this.connsDebug("giving connection")
    ...
    createConnection(options, cb) {
        ...
        if (!sock) {
            this.connsDebug("socket in queue")
